...
let firstName = id("firstName"),
    ...
    gender = "",
    notes = id("floatingTextarea"),
    form = id("form"),
    table = id("table"),
    RemoveBtn = id("removeBtn"),
    errorMassage = document.getElementsByClassName("error")
...
form.addEventListener('submit', (e) => {
    ...
    giveError(firstName, 0, "put your firstname here, required!")
    giveError(lastName, 1, "put your lastname here, required!")
    giveError(address, 2, "put your address here, required!")

    if(firstName.value.length == 0 || lastName.value.length == 0 || address.value.length == 0){
...
const showData = () => {
    ...
    arr.map((item) => {
        let index = item.id
        let btnRemove = `
            <button onclick=showPopup(${index}) id="myBtn" type="button" class="btn btn-success"><i class="fa">&#xf06e;</i></button>
            <button onclick=removeItem(${index}) type="button" class="btn btn-danger">X</button>
        `
